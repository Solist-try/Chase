...
export class GameEngine {
  ...
  update(delta) {
    ...
    if (this.dragon.y + this.dragon.height > this.canvas.height + 20) {
      this.#loseGame('void');
    }
  ...
  #loseGame(reason = 'foe') {
    ...
    // Keep the loop running so the lose banner can paint (same as win)
    this.hearts = 0;
    this.updateHud();
  ...
  render() {
    ...
    // Lose banner (void fall, special foe, …)
    if (this.lost) {
      this.ctx.fillStyle = 'rgba(27, 42, 74, 0.5)';
      this.ctx.fillRect(0, 0, this.canvas.width, this.canvas.height);

      this.ctx.fillStyle = '#fffaf0';
      this.ctx.font = 'bold 28px Fredoka, Nunito, sans-serif';
      this.ctx.textAlign = 'center';
      this.ctx.fillText('Oh no!', this.canvas.width / 2, this.canvas.height / 2 - 10);

      this.ctx.font = 'bold 16px Nunito, sans-serif';
      this.ctx.fillText(
        this.loseReason === 'void'
          ? 'You fell into the void!'
          : 'The special foe got you!',
        this.canvas.width / 2,
        this.canvas.height / 2 + 22,
      );
    }
